...
list_data_3G= []
list_data_2G= []
list_data_sisa= [] #kalau ada NL diluar 4G 3G 2G Huawei dan ERI
...
for daftar_file in os.listdir(folder_proses):
	...
	##pengaman kalau ada yang missing
	nama_oss = " "
	jumlah_file_terproses = " "
	status_task = " "
	list_jumlah_sector = [0]    #khusus untuk menampung regex jumlah sector sebelum dijumlahkan
	missing_sector = " "
	missing_RET = " "
	...
	if "4g" in nama_oss :
		...
		list_data_3G.append(nama_oss +"%"+ jumlah_file_terproses +"%"+ status_task +"%"+ str(jumlah_sector) +"%"+ missing_sector +"%"+ missing_RET ) #% = buat pembatas
	...
	elif "Ran" in nama_oss :
		list_data_ericsson.append(nama_oss +"%"+ jumlah_file_terproses +"%"+ status_task +"%"+ str(jumlah_sector) +"%"+ missing_sector +"%"+ missing_RET ) #% = buat pembatas

	#kalau ada NL diluar 4G 3G 2G Huawei dan ERI
	elif "/" in nama_oss :
		list_data_sisa.append(nama_oss +"%"+ jumlah_file_terproses +"%"+ status_task +"%"+ str(jumlah_sector) +"%"+ missing_sector +"%"+ missing_RET ) #% = buat pembatas

	else :
		print ("^ ini source nya ngga ada, bisa jadi parent task, dilewat dulu ya")
		
		
## Mengurutkan sesuai alfabet
list_data_ericsson.sort()
list_data_4G.sort()
list_data_3G.sort()
list_data_2G.sort()
list_data_sisa.sort()
...
for item in list_data_ericsson :
